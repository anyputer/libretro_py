...
void NETRETROPAD_CORE_PREFIX(retro_init)(void)
{
   ...
   py_buffer.buf = frame_buf;
   py_buffer.len = WIDTH * HEIGHT * sizeof(uint16_t);
   py_buffer.itemsize = 2;
   py_buffer.format = "H";
   py_buffer.ndim = 1;

   py_memview = PyMemoryView_FromBuffer(&py_buffer);

   if (frame_buf)
   {
      uint16_t *pixel = frame_buf + 49 * WIDTH + 32;
      ...
      struct retro_system_info *info)
{
   ...
   info->library_name     = "button_test_py";
...
void NETRETROPAD_CORE_PREFIX(retro_run)(void)
{
   ...
   */
   ...
   py_retro_run(input_state);

   NETRETROPAD_CORE_PREFIX(video_cb)(frame_buf, WIDTH, HEIGHT, STRIDE);
}
